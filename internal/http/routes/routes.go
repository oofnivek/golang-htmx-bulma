...
func RegisterRoutes(
	...
	if vcHandler != nil || vmHandler != nil || vtHandler != nil || vsHandler != nil || ftHandler != nil || vmdlHandler != nil || vfHandler != nil || fcHandler != nil || caoHandler != nil || cpoHandler != nil || cpHandler != nil || cplHandler != nil || estateHandler != nil || riHandler != nil || vHandler != nil || fuelCardHandler != nil || condoHandler != nil || condoCarParkHandler != nil || condoPostalCodeHandler != nil || condoAcknowledgementHandler != nil || vgsHandler != nil || roleHandler != nil || userHandler != nil {
		...
		protected.Use(middleware.AuthMiddleware(signingKey))
		{
			...
			// Vehicles
			if vHandler != nil {
				vGroup := protected.Group("/vehicles")
				{
					vGroup.GET("", vHandler.List)
					vGroup.GET("/new", vHandler.CreateForm)
					vGroup.POST("", vHandler.Create)
					vGroup.GET("/:id/view", vHandler.View)
					vGroup.GET("/:id/edit", vHandler.EditForm)
					vGroup.POST("/:id", vHandler.Update)
					vGroup.DELETE("/:id", vHandler.Delete)
					vGroup.GET("/:id/delete", vHandler.DeleteConfirm)
				}
			}
			...
			// Condo Postal Codes
			if condoPostalCodeHandler != nil {
				ccpcGroup := protected.Group("/condo-postal-codes")
				{
					ccpcGroup.GET("", condoPostalCodeHandler.List)
					ccpcGroup.GET("/new", condoPostalCodeHandler.CreateForm)
					ccpcGroup.POST("", condoPostalCodeHandler.Create)
					ccpcGroup.GET("/:id/view", condoPostalCodeHandler.View)
					ccpcGroup.GET("/:id/edit", condoPostalCodeHandler.EditForm)
					ccpcGroup.POST("/:id", condoPostalCodeHandler.Update)
					ccpcGroup.DELETE("/:id", condoPostalCodeHandler.Delete)
					ccpcGroup.GET("/:id/delete", condoPostalCodeHandler.DeleteConfirm)
				}
			}
	...
	apiGroup := r.Group("/api")
	{
		...
		if vAPI != nil {
			apiGroup.GET("/vehicles/all", vAPI.ListAll)
			apiGroup.GET("/vehicles", vAPI.List)
			apiGroup.GET("/vehicles/:id", vAPI.Get)
		...
		if condoPostalCodeAPI != nil {
			apiGroup.GET("/condo-postal-codes/all", condoPostalCodeAPI.ListAll)
			apiGroup.GET("/condo-postal-codes", condoPostalCodeAPI.List)
			apiGroup.GET("/condo-postal-codes/:id", condoPostalCodeAPI.Get)
