...
func (r *mysqlVehicleTypeRepository) GetAll() ([]VehicleType, error) {
    rows, err := r.db.Query("SELECT id, name, status, created_by, created_at, updated_by, updated_at FROM vehicle_type ORDER BY id DESC")
    if err != nil { return nil, err }
    defer rows.Close()
    var types []VehicleType
    for rows.Next() {
        var vt VehicleType
        if err := rows.Scan(&vt.ID, &vt.Name, &vt.Status, &vt.CreatedBy, &vt.CreatedAt, &vt.UpdatedBy, &vt.UpdatedAt); err != nil {
            return nil, err
        }
...
func (r *mysqlVehicleTypeRepository) GetPaged(limit, offset int, sortBy, sortOrder string) ([]VehicleType, error) {
    ...
    query := "SELECT id, name, status, created_by, created_at, updated_by, updated_at FROM vehicle_type ORDER BY " + sortBy + " " + sortOrder + " LIMIT ? OFFSET ?"
...
func (r *mysqlVehicleTypeRepository) GetByID(id int64) (*VehicleType, error) {
    row := r.db.QueryRow("SELECT id, name, status, created_by, created_at, updated_by, updated_at FROM vehicle_type WHERE id = ?", id)
    var vt VehicleType
    err := row.Scan(&vt.ID, &vt.Name, &vt.Status, &vt.CreatedBy, &vt.CreatedAt, &vt.UpdatedBy, &vt.UpdatedAt)
...
func (r *mysqlVehicleTypeRepository) Create(vt *VehicleType) error {
    res, err := r.db.Exec("INSERT INTO vehicle_type (name, status, created_by, created_at, updated_by, updated_at) VALUES (?,?,?,?,?,?)",
        vt.Name, vt.Status, vt.CreatedBy, vt.CreatedAt, vt.UpdatedBy, vt.UpdatedAt)
...
func (r *mysqlVehicleTypeRepository) Update(vt *VehicleType) error {
    _, err := r.db.Exec("UPDATE vehicle_type SET name = ?, status = ?, updated_by = ?, updated_at = ? WHERE id = ?",
        vt.Name, vt.Status, vt.UpdatedBy, vt.UpdatedAt, vt.ID)
    return err
}
